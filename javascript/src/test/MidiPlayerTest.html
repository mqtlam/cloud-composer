...
<html>
    <head>
        <title>Test Page for Midi Player</title>
    ...
    <body>
        ...
        <script language="javascript">
            function testReset() {
                ...
                // confirm highlight bar moved back to start
                // confirm mode is set to "not playing"
                // confirm song position set to 0
                assertTrue(document.getElementById("column0").className == "highlightbar" 
                    && !document.getElementById("midiplayer").inPlayMode
                    && document.getElementById("midiplayer").basePosition == 0);
            }
            function testUpdateBpmValueOnPage() {
                    $('$tempo').slider('option', 'value', 50); // set slider value

                    update_bpmValueOnPage();
                    
                    var bpm = document.getElementById("sliderValue");
                    assertEquals(bpm, "BPM = 50");
            }
        </script>
    </body>
</html>
