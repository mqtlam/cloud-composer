...
<html>
    <head>
        <title>Test Page for Instrument Selector</title>
    ...
    <body>
        ...
        <script language="javascript">
            function testAddInstrument() {
                var ib = document.getElementById("instrumentBar");
                
                ib.addInstrument("foo", "", white);
                assertTrue(ib.instruments[0] && ib.instruments[0].instrumentName == "foo");
            }
            function testRemoveInstrument() {
                var ib = document.getElementById("instrumentBar");
                
                ib.addInstrument("foo", "", white); // add to list
                ib.removeInstrument("foo", "", white); // remove from list
                assertTrue(ib.instruments.length == 0); // list should be empty now
            }
            function testSetActiveInstrument() {
                var ib = document.getElementById("instrumentBar");
                ib.addInstrument("piano", "", white);
            
                ib.setActiveInstrument("piano");
                assertEquals(ib.currentInstrument.instrumentName, "piano");
            }
        </script>
    </body>
</html>
